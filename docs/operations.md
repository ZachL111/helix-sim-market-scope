...
`helix-sim-market-scope` is centered on simulations work in C#. The technical target is: Create a C# reference implementation for market workflows, centered on state machine modeling, transition tables, and invalid-transition tests.
...
The score is deterministic. The current threshold is 172. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 6.
